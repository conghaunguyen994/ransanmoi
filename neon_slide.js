...
function initNeonSlide(containerId) {
    ...
    const TILE = 90;
    const GAP = 6;
    const BOARD = N * TILE + (N + 1) * GAP; // 4*90 + 5*6 = 390
    const W = BOARD;
    const H = BOARD + 64;
    ...
    // Sinh trạng thái tạm bằng cách tráo từ trạng thái giải để đảm bảo có lời giải
    function shuffleSolvable() {
        ...
        let blank = N * N - 1;
        let prevDir = -1;
        const dirs = [[0,-1],[0,1],[-1,0],[1,0]]; // blank di chuyển
        for (let step = 0; step < 200; step++) {
            const [br, bc] = rc(blank);
            const candidates = [];
            dirs.forEach((d, di) => {
                if (di === (prevDir ^ 1)) return; // tránh đảo lại nước vừa đi
    ...
    function newPuzzle() {
        state = {
            ...
            moves: 0,
            startTime: Date.now(),
            anim: null, // {value, fromR, fromC, toR, toC, t}
            animId: 0
        };
    ...
    canvas.addEventListener('click', (e) => {
        ...
        if (my < BOARD) clickCell(mx, my);
            ...
            // nút "Xáo lại" nằm giữa board và đáy
            newPuzzle();
        }
    });
    ...
    function onKey(e) {
        ...
        const [br, bc] = rc(state.blank);
        let target = -1;
        // Mũi tên = hướng ô cần TRƯỢT vào ô trống
